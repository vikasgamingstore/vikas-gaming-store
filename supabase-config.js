...
/* ---------- product images ---------- */
/* Returns the gallery as an array. Falls back to the older single
   image_url column so products saved before galleries still work. */
function productImages(p){
  let list = [];
  if(Array.isArray(p.image_urls)) list = p.image_urls.filter(Boolean);
  else if(typeof p.image_urls === "string"){
    try { const parsed = JSON.parse(p.image_urls); if(Array.isArray(parsed)) list = parsed.filter(Boolean); } catch(e){}
  }
  if(list.length === 0 && p.image_url) list = [p.image_url];
  return list;
}
...
function addToCart(product, qty){
  ...
  if(existing){ existing.qty += qty; }
    ...
    cart.push({
      ...
      image_url: mainImage(product), condition: product.condition,
